...
class CCCourseTestCase extends UnitTestCase {
    ...
    function test_createFromCoursesMessage_allinone() {
        $db = DBManager::get();
        $anzahl = $db->query("SELECT COUNT(*) FROM seminare")->fetch(PDO::FETCH_COLUMN, 0);
        $this->assertEqual($anzahl, 0);
        ...
        $data['import_settings']['course_entity_type'] = "kurs";
        $participant['data'] = $data;
        $participant->store();
        $message = array(
            'lectureID' => "23",
        ...
        CCCourse::createFromCoursesMessage($message, $participant->getId());
        $seminare = $db->query("SELECT * FROM seminare")->fetchAll(PDO::FETCH_ASSOC);
        $this->assertEqual(count($seminare), 1);
        $this->assertEqual($seminare[0]['Name'], "Test 5");
        $anzahl = $db->query("SELECT COUNT(*) FROM statusgruppen")->fetch(PDO::FETCH_COLUMN, 0);
    ...
    function test_createFromCoursesMessage_many() {
        $db = DBManager::get();
        $anzahl = $db->query("SELECT COUNT(*) FROM seminare")->fetch(PDO::FETCH_COLUMN, 0);
        $this->assertEqual($anzahl, 0);
        ...
        $participant = new CCParticipant();
        $data = array();
        $data['import_settings']['cms']['parallelgroups'] = "many";
        $data['import_settings']['course_entity_type'] = "cms";
        $participant['data'] = $data;
        $participant->store();
        CCCourse::createFromCoursesMessage($message, $participant->getId());
        $anzahl = $db->query("SELECT COUNT(*) FROM seminare")->fetch(PDO::FETCH_COLUMN, 0);
        $this->assertEqual($anzahl, 4);
        $anzahl = $db->query("SELECT COUNT(*) FROM statusgruppen")->fetch(PDO::FETCH_COLUMN, 0);
        $this->assertEqual($anzahl, 0);
