...
class CCCourse extends Course
{
    ...
    static public function createFromCoursesMessage($message, $participant_id)
    {
        $participant = new CCParticipant($participant_id);
        if (!in_array($participant['data']['import_settings']['course_entity_type'], array("kurs", "cms"))) {
            return false;
        }

        $seminar_ids = array();
        if ($message['groupScenario'] < 2) {
            $seminar_ids = self::createCourse(
                $message,
                $message['title'],
                $message['lectureID'],
                $message['groupScenario'] == 1 ? $message['groups'] : array(),
                null,
                $participant_id
            );
        } elseif($message['groupScenario'] == 2) {
            $seminar_ids = self::createCourse(
                $message,
                $message['title'], //title
                $message['lectureID'], //id
                array(), //groups - no groups
                null,
                $participant_id
            );
            foreach ((array) $message['groups'] as $id => $group) {
                $seminar_ids = array_merge(
                    $seminar_ids,
                    self::createCourse(
                        $message,
                        $group['title']." - ".($id + 1), //title
                        $message['lectureID']."-".$id, //id
                        array(), //groups - no groups
                        $group['id'],
                        $participant_id
                    )
        ...
        } elseif($message['groupScenario'] == 3) {
            $lecturers = array();
            foreach ($message['groups'] as $group) {
                $lecturers = array_merge($lecturers, array_map(function($l) {
                    return $l['firstName']." ".$l['lastName'];},
                     $group['lecturers']));
            }
            $lecturers = array_unique($lecturers);
            foreach ($lecturers as $lecturer) {
                $title = $message['title']. " - " . $lecturer;
                $groups = array();
                foreach ($message['groups'] as $parallelgroup) {
                    foreach ($parallelgroup['lecturers'] as $grouplecturer) {
                        if ($grouplecturer['firstName']." ".$grouplecturer['lastName'] === $lecturer) {
                ...
                $seminar_ids = array_merge(
                    ...
                    self::createCourse(
                        $message,
                        $title, //title
                        $message['lectureID']."-".$lecturer, //id
                        $groups, //all subgroups belonging to this teacher
                        null,
                        $participant_id
                    )
                );
        ...
        $mapping = new CampusConnectEntity(array($message['lectureID'], "coursegroup"));
        $mapping['foreign_id'] = $message['lectureID'];
        $mapping['participant_id'] = $participant_id;
        $mapping['data'] = $message;
        $mapping->store();

        $db = DBManager::get();
        $old_seminar_ids = $db->query(
            "SELECT Seminar_id " .
            "FROM campus_connect_course_group " .
            "WHERE cg_id = ".$db->quote($mapping['item_id'])." " .
        "")->fetchAll(PDO::FETCH_COLUMN, 0);
        foreach (array_diff(array_keys($seminar_ids), $old_seminar_ids) as $new_seminar_id) {
            $db->exec(
                "INSERT IGNORE INTO campus_connect_course_group " .
                "SET cg_id = ".$db->quote($mapping['item_id']).", " .
                    "Seminar_id = ".$db->quote($new_seminar_id).", " .
                    "parallelgroup_id = ".$db->quote($seminar_ids[$new_seminar_id] !== 1 ? $seminar_ids[$new_seminar_id] : null)." " .
            "");
        }
        foreach (array_diff($old_seminar_ids, array_keys($seminar_ids)) as $lost_seminar_id) {
            $db->exec(
                "DELETE FROM campus_connect_course_group " .
                "WHERE cg_id = ".$db->quote($mapping['item_id'])." " .
                    "AND Seminar_id = ".$db->quote($new_seminar_id)." " .
            "");
            $course = new CCCourse($lost_seminar_id);
            $course->delete();
        }
        return $seminar_ids;
    }
    ...
    /**
     * Creates and updates the coursemembers of a course from a coursemembermessage.
     * This treats imported coursemembers as a set and tries to update that whole set.
     * That means it remembers who has been imported in the first place and deletes
     * everyone from this set who does not appear in the new coursemember set.
     * Everyone who takes part in the course, but has not been imported via LSF
     * will stay in the course even if he/she does not appear in the new
     * coursemember set.
     * And of course new coursemembers will be imported, already existing
     * coursemembers' statusses will be updated.
     *
     * This could only cause problems, if on LSF-side a user is accidently added
     * to a course, who has been into that course in Stud.IP-side before. And when
     * that accident on LSF-side is recognized and the user once again is deleted
     * on LSF-side, he/she will also be deleted on Stud.IP-side although he/she
     * had been in the course before that accident happened. But that should be
     * a rather small gap. And the user in Stud.IP could still register to that
     * course again and everything should be fine.
     *
     * @param array $message
     * @param integer $participant_id
     */
    static public function createCourseMembers($message, $participant_id)
    {
        $participant = new CCParticipant($participant_id);
        ...
        $db = DBManager::get();
        $seminar_data = $db->query(
            "SELECT * " .
            "FROM campus_connect_course_group " .
            "WHERE cg_id = ".$db->quote($message['lectureID'])." " .
        "")->fetchAll(PDO::FETCH_ASSOC);

        //Als erstes alle Seminare der course_group (Parallelgruppen inklusive) finden
        //und f�r jedes entscheiden, ob der Nutzer eingetragen sein sollte oder nicht.
        foreach ($seminar_data as $seminar) {
            ...
            $old_synced_coursemembers = $course['members']->getArrayCopy();
            foreach ($old_synced_coursemembers as $key => $member) {
                $mapped_coursemember = CampusConnectEntity::findBySQL(
                    "type = 'coursemember' AND item_id = ? AND participant_id = ?",
                    array(
                        $member['Seminar_id']."-".$member['user_id'],
                        $participant_id
                    )
                );
                if (!$mapped_coursemember) {
                    unset($old_synced_coursemembers[$key]);
                }
            }
            //Alle Kursteilnehmer, __die je importiert wurden__, sollen komplett abgeglichen werden
            foreach ($message['members'] as $member) {
                ...
                switch ($idtype) {
                    case "username":
                        ...
                        break;
                    case "email":
                        $user = User::findByEmail($member['personID']);
                ...
                if ($user) {
                    //erst einmal herausfinden, ob es dieses Seminar �berhaupt sein soll.
                    $insert_into_group = false;
                    if ($participant['data']['import_settings']['cms']['parallelgroups'] === "allinone") {
                        $insert_into_group = true;
                    } elseif ($participant['data']['import_settings']['cms']['parallelgroups'] === "many") {
                        if ($seminar['parallelgroup_id'] !== null) {
                            foreach ((array) $member['groups'] as $group) {
                                if ($group['id'] == $seminar['parallelgroup_id']) {
                                    $insert_into_group = true;
                                }
                            }
                        } else {
                    ...
                    } elseif($participant['data']['import_settings']['cms']['parallelgroups'] === "dozent") {
                        foreach ((array) $member['groups'] as $group) {
                            foreach ((array) $course_mapping['statusgruppen'] as $statusgruppe) {
                                if ($group['id'] == $statusgruppe['id']) {
                                    $insert_into_group = true;
                                }
                            }
                        }
                    ...
                    if ($insert_into_group) {
                        //Generiere Kursmember
                        $vorhanden = $course['members']->findBy('user_id', $user->id)->first();
                        if (!$vorhanden) {
                            $coursemember = new CourseMember();
                            $coursemember['user_id'] = $user->getId();
                            $coursemember['status'] = $rights[$member['role']];
                            $course['members'][] = $coursemember;
                        }

                        foreach ($old_synced_coursemembers as $key => $member1) {
                            if ($member1['user_id'] === $user->getId()) {
                                unset($old_synced_coursemembers[$key]);
                            }
                        }

                        $mapping = new CampusConnectEntity(array($course->getId()."-".$user->getId(), "coursemember"));
                        $mapping['foreign_id'] = $message['lectureID']."-".$member['personID'];
                        $mapping['participant_id'] = $participant_id;
                        $mapping['data'] = $message;
                        $mapping->store();
                    }
            ...
            foreach ($old_synced_coursemembers as $member1) {
                foreach ($course['members'] as $key => $member2) {
                    if (($member1['Seminar_id'] === $member2['Seminar_id']) && ($member1['user_id'] === $member2['user_id'])) {
                        unset($course['members'][$key]);
                        $mapping = new CampusConnectEntity(array($course->getId()."-".$member1['user_id'], "coursemember"));
                        $mapping->delete();
                    }
                }
            }

            //Dummy-Dozent behandeln
            $dummy_dozent = self::getDummyDozent();
            $dozent_vorhanden = false;
            $dummydozent_vorhanden = false;
            foreach ($course['members'] as $key => $member) {
                if ($member['user_id'] === $dummy_dozent['user_id']) {
                    $dummydozent_vorhanden = $key;
                } elseif (($member['status'] === "dozent") && ($member['user_id'] !== $dummy_dozent['user_id'])) {
                    $dozent_vorhanden = true;
                }
            }

            if ($dozent_vorhanden && $dummydozent_vorhanden !== false) {
                unset($course['members'][$dummydozent_vorhanden]);
            }
            if (!$dozent_vorhanden && $dummydozent_vorhanden === false) {
                $course['members'][] = $dummy_dozent;
            }
